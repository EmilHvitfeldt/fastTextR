...
fasttext <- function() {
    ...
    model$sentence_vectors <- function(sentences) {
        ft_sentence_vectors(parent.env(environment())$model, sentences)
    }
...
#' }
ft_sentence_vectors <- function(model, sentences) {
    stopifnot(is.character(sentences), all(nchar(sentences) > 0))
    stopifnot( inherits(model, "fasttext") )
    sentence_vector <- Rft_sentence_vectors(model$pointer, sentences)
    sentence_vector <- do.call(rbind, sentence_vector)
    rownames(sentence_vector) <- sentences
    sentence_vector
}
